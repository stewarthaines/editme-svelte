    ...
    {#each spineItems as item, index (item.id)}
      <div class="spine-item" class:dragging={item.isDragging}>
        ...
        <div class="item-actions">
          ...
          <button on:click={() => moveDown(index)} disabled={index === spineItems.length - 1}>↓</button>
  ...
  <div class="create-spine-item-form">
    ...
    <div class="field">
      <label>
        <input 
          type="checkbox" 
          bind:checked={newItem.createSource}
        />
...
const getDisplayTitle = (item: SpineItemWithSource): string => {
  ...
  const baseName = item.href.split('/').pop()?.replace(/\.(x?html?)$/, '')
