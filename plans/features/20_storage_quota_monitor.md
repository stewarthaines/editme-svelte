...
<div class="storage-quota-monitor">
  ...
  {:else if quotaInfo}
                  ...
                  {#each usageBreakdown.workspaces as workspace}
                    <div class="workspace-item">
                      ...
                      <div class="workspace-actions">
                        <button on:click={() => viewWorkspace(workspace.id)}>
                          View
                        </button>
    ...
    <div class="quota-actions">
      <button on:click={refreshQuotaInfo}>
        Refresh
      </button>
      <button on:click={() => showDetails = !showDetails}>
        {showDetails ? 'Hide' : 'Show'} Details
      </button>
      <button on:click={() => showCleanupDialog = true}>
        Free Up Space
      </button>
    </div>
  {:else}
...
const generateCleanupSuggestions = async (): Promise<CleanupSuggestion[]> => {
  ...
  if (usageBreakdown.cache > 1024 * 1024) { // > 1MB cache
    suggestions.push({
      type: 'cache',
...
const canPerformOperation = async (operationType: string, estimatedSize: number): Promise<boolean> => {
  try {
    const quotaInfo = await getQuotaInfo()
