...
<Story
  ...
  parameters={{
    docs: {
      description: {
        story:
          'Empty workspace showing the initial state when starting a new EPUB project.',
      },
    },
  }}
/>
...
<Story
  ...
  parameters={{
    docs: {
      description: {
        story:
          'Application view with sidebar collapsed to maximize content editing space.',
      },
    },
  }}
  play={async ({ canvasElement }) => {
...
<Story
  ...
  play={async ({ canvasElement }) => {
    ...
    try {
      ...
      const chapterButton = spineItems.find(btn => 
        btn.textContent?.includes('chapter-001') || btn.textContent?.includes('Chapter 1')
      );
      ...
      if (chapterButton) {
        ...
        console.log('No chapter button found in:', spineItems.map(btn => btn.textContent));
      }
    } catch (error) {
...
<Story
  ...
  play={async ({ canvasElement }) => {
    ...
    try {
      ...
      const chapterButton = allButtons.find(btn => 
        btn.textContent?.includes('chapter-001')
      );
