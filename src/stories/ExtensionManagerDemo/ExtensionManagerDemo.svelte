...
<script lang="ts">
  ...
  // Sample data
  import { SAMPLE_EXTENSIONS, getPopularExtensions, createSampleFile } from './mock-data/sample-extensions.js';
  ...
  // Workflow selection
  const workflows = [
    { id: 'basic-import', name: 'Basic Extension Import', description: 'Upload → Validate → Import → Cache' },
    { id: 'cache-management', name: 'Cache Management', description: 'Browse Cache → Import → Conflict Detection' },
    { id: 'multi-file', name: 'Multi-File Extensions', description: 'Import Base → Add Files → Manage Complete Extension' },
    { id: 'workspace-import', name: 'Workspace Import', description: 'EPUB Upload → Auto-Scan → Batch Cache → Summary' },
    { id: 'extension-library', name: 'Extension Library', description: 'Browse → Organize → Batch Operations → Export' }
  ];
  ...
  async function refreshData() {
    ...
    try {
      ...
      state.workspaceExtensions = await extensionManager.listWorkspaceExtensions(state.selectedWorkspace);
      ...
      log('info', `Loaded ${state.workspaceExtensions.length} workspace extensions, ${state.cachedExtensions.length} cached extensions`);
    } catch (error) {
  ...
  async function updateStorageQuota() {
    try {
      ...
      state.storageQuota = {
        used: cacheStats.totalSize + state.workspaceExtensions.reduce((sum, ext) => sum + ext.totalSize, 0),
  ...
  async function importSingleFile(file: File) {
    ...
    try {
      ...
      const result = await extensionManager.importExtension(state.selectedWorkspace, file, detectedName);
  ...
  // Delete extension
  async function handleExtensionDelete(event: CustomEvent<{ extensionName: string; location: 'workspace' | 'cache' }>) {
  ...
  // Batch operations
  async function handleBatchOperation(event: CustomEvent<{ operation: string; extensions: string[] }>) {
  ...
  async function simulateEPUBImport(epubName: string) {
    ...
    log('success', `EPUB import complete: ${summary.successCount} extensions cached, ${summary.conflicts.length} conflicts`);
