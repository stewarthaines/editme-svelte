...
vi.mock('./utils.js', () => ({
	bufferToStream: vi.fn((buffer) => new ReadableStream({
		start(controller) {
...
describe('ZipWriter', () => {
	...
	describe('addFile', () => {
		...
		it('should reject unsupported data type', async () => {
			...
			await expect(zipWriter.addFile('test.txt', unsupportedData as any))
				.rejects.toThrow('Unsupported data type');
		});

		it('should reject unsupported compression method', async () => {
			await expect(zipWriter.addFile('test.txt', 'Hello World', { 
	...
	describe('build', () => {
		...
		it('should place mimetype file first', async () => {
			...
			const fileName = new TextDecoder().decode(
				new Uint8Array(zipBuffer, 30, fileNameLength)
			);
	...
	describe('integration scenarios', () => {
		it('should handle realistic EPUB creation workflow', async () => {
			...
			// Add META-INF files
			await writer.addFile('META-INF/container.xml', 
				'<?xml version="1.0" encoding="UTF-8"?>\n<container version="1.0">...</container>');
			
			// Add OEBPS content
			await writer.addFile('OEBPS/content.opf', 
				'<?xml version="1.0" encoding="UTF-8"?>\n<package>...</package>');
			await writer.addFile('OEBPS/toc.ncx', 
				'<?xml version="1.0" encoding="UTF-8"?>\n<ncx>...</ncx>');
			
			// Add chapters
			for (let i = 1; i <= 3; i++) {
				await writer.addFile(`OEBPS/chapter${i}.xhtml`, 
					`<html><head><title>Chapter ${i}</title></head><body><h1>Chapter ${i}</h1></body></html>`);
			}
			
			// Add CSS
			await writer.addFile('OEBPS/styles.css', 
