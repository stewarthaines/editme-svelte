...
export class MockOPFSDirectoryHandle implements OPFSDirectoryHandle {
	...
	async getDirectoryHandle(name: string, options?: { create?: boolean }): Promise<MockOPFSDirectoryHandle> {
...
export class TestHelpers {
	static assertArrayBuffersEqual(actual: ArrayBuffer, expected: ArrayBuffer): void {
		...
		if (actualView.length !== expectedView.length) {
			throw new Error(`ArrayBuffer length mismatch: ${actualView.length} vs ${expectedView.length}`);
		}
		
		for (let i = 0; i < actualView.length; i++) {
			if (actualView[i] !== expectedView[i]) {
				throw new Error(`ArrayBuffer content mismatch at byte ${i}: ${actualView[i]} vs ${expectedView[i]}`);
			}
		}
	}
