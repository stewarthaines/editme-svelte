...
export class ExtensionCache {
  ...
  async importFromCache(extensionName: string, targetWorkspaceId: string): Promise<void> {
    ...
    const hasConflict = workspaceFiles.some(f =>
      f.startsWith(`SOURCE/extensions/${extensionName}/`)
    );
   ...
   */
  async compareExtensions(
    workspaceId: string,
    workspaceExtensionName: string,
    cacheExtensionName: string
  ): Promise<boolean> {
    try {
      ...
      const cacheExtensionFiles = cacheFiles.filter(f => f.startsWith(`${cacheExtensionName}/`));
      ...
      const workspaceSignature = await this.createWorkspaceSignature(
        workspaceId,
        workspaceExtensionName,
        workspaceExtensionFiles
      );
      const cacheSignature = await this.createCacheSignature(
        cacheExtensionName,
        cacheExtensionFiles
      );
   ...
   */
  private async createWorkspaceSignature(
    workspaceId: string,
   ...
   */
  private async createCacheSignature(
    extensionName: string,
    files: string[]
  ): Promise<ExtensionSignature> {
    const fileInfos = [];
