...
describe('ExtensionManager', () => {
  ...
  describe('importExtension()', () => {
    ...
    it('should normalize extension names consistently', async () => {
      ...
      const workspaceFiles = mockFileStorage.getWorkspaceFiles(workspaceId);
      expect(workspaceFiles.has('SOURCE/extensions/my-custom-library/My Custom Library.js')).toBe(
        true
      );
    });
  ...
  describe('listWorkspaceExtensions()', () => {
    ...
    it('should handle storage read errors', async () => {
      ...
      await expect(extensionManager.listWorkspaceExtensions(workspaceId)).rejects.toThrow(
  ...
  describe('listCachedExtensions()', () => {
    ...
    it('should handle cache access errors', async () => {
      ...
      await expect(extensionManager.listCachedExtensions()).rejects.toThrow('Failed to list files');
    });
  });
  ...
  describe('importFromCache()', () => {
    ...
    it('should throw error for non-existent cache entry', async () => {
      ...
      await expect(extensionManager.importFromCache(workspaceId, 'non-existent')).rejects.toThrow(
    ...
    it('should throw error for workspace conflicts', async () => {
      ...
      await expect(extensionManager.importFromCache(workspaceId, 'markdown-it')).rejects.toThrow(
  ...
  describe('deleteCachedExtension()', () => {
    ...
    it('should throw error for non-existent cache entry', async () => {
      await expect(extensionManager.deleteCachedExtension('non-existent')).rejects.toThrow(
        "Extension 'non-existent' not found in cache"
      );
    });
  });
  ...
  describe('cacheExtension()', () => {
    ...
    it('should throw error for cache conflicts with different content', async () => {
      ...
      await expect(extensionManager.cacheExtension(workspaceId, 'markdown-it')).rejects.toThrow(
    ...
    it('should throw error for non-existent workspace extension', async () => {
      ...
      await expect(extensionManager.cacheExtension(workspaceId, 'non-existent')).rejects.toThrow(
