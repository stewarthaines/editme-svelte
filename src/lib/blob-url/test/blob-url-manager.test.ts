...
describe('BlobURLManager', () => {
  ...
  describe('createBlobURL() - OPFS Path', () => {
    ...
    it('should create blob URL using OPFS zero-copy path', async () => {
      const mockFile = new File(['test content'], 'cover.jpg', { type: 'wrong/type' });
    ...
    it('should throw error when file not found', async () => {
      ...
      await expect(manager.createBlobURL('missing/file.jpg'))
        .rejects.toThrow(BlobURLError);
      
      await expect(manager.createBlobURL('missing/file.jpg'))
        .rejects.toThrow('Failed to create blob URL for missing/file.jpg: File not found');
    });
  });
  ...
  describe('createBlobURL() - IndexedDB Path', () => {
    ...
    it('should create blob URL using IndexedDB memory-copy path', async () => {
      ...
      expect(mockFileStorage.readFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/images/cover.jpg');
      expect(mockCreateObjectURL).toHaveBeenCalledWith(
        expect.objectContaining({
          type: 'image/jpeg'
        })
  ...
  describe('Capacity Management', () => {
    ...
    it('should throw capacity error when limit exceeded', async () => {
      ...
      await expect(manager.createBlobURL('image3.jpg'))
        .rejects.toThrow(BlobURLCapacityError);
      
      await expect(manager.createBlobURL('image3.jpg'))
        .rejects.toThrow('Blob URL capacity exceeded: 2/2');
    });
