  ...
  parseFromString(content: string, mimeType: string) {
    ...
    const mockDoc = {
      ...
      querySelectorAll: vi.fn((selector: string) => {
        ...
        if (selector.includes('video[src]') && content.includes('<video') && content.includes('src=')) {
...
describe('XHTML Processing', () => {
  ...
  describe('Asset Element Detection', () => {
    it('should process script[src] elements', async () => {
      ...
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/scripts/reader.js');
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/scripts/utils.js');
    });
    ...
    it('should process link[href] elements', async () => {
      ...
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/images/favicon.ico');
    });
    ...
    it('should process img[src] elements', async () => {
      ...
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/images/photo.png');
    });
    ...
    it('should process video[src] and video[poster] elements', async () => {
      ...
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/media/intro.mp4');
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/images/poster.jpg');
    });
    ...
    it('should process audio[src] elements', async () => {
      ...
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/audio/narration.mp3');
    });
    ...
    it('should process a[href] elements', async () => {
      ...
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/chapter2.xhtml');
    });
    ...
    it('should process object[data] elements', async () => {
      ...
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/images/diagram.svg');
    });
    ...
    it('should process image[href] elements (SVG)', async () => {
      ...
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/images/embedded.jpg');
    });
    ...
    it('should process [data-src] elements', async () => {
      ...
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/images/lazy-load.jpg');
    });
  });
  ...
  describe('URL Classification', () => {
    it('should only process relative URLs', async () => {
      ...
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/styles/main.css');
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/scripts/local.js');
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/images/local.jpg');
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/chapter2.xhtml');

      // Should NOT process external/special URLs
      expect(mockFileStorage.getFile).not.toHaveBeenCalledWith(expect.anything(), expect.stringContaining('example.com'));
      expect(mockFileStorage.getFile).not.toHaveBeenCalledWith(expect.anything(), expect.stringContaining('data:'));
      expect(mockFileStorage.getFile).not.toHaveBeenCalledWith(expect.anything(), expect.stringContaining('blob:'));
    });
  ...
  describe('Error Handling', () => {
    it('should throw XHTMLProcessingError for malformed XHTML', async () => {
      ...
      await expect(manager.processXHTMLForPreview(invalidXHTML))
    ...
    it('should throw capacity error when limit reached during processing', async () => {
      const capacityManager = new BlobURLManager({ ...testConfig, maxBlobURLs: 1 });
      ...
      await expect(capacityManager.processXHTMLForPreview(xhtml))
        .rejects.toThrow(BlobURLCapacityError);
    });

    it('should check capacity before processing', async () => {
      const capacityManager = new BlobURLManager({ ...testConfig, maxBlobURLs: 0 });
      capacityManager.setActiveWorkspace('test-workspace');
