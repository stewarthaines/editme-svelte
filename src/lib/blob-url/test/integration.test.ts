...
describe('Blob URL Manager Integration', () => {
  ...
  describe('Workspace Integration', () => {
    it('should integrate with WorkspaceManager for path configuration', async () => {
      ...
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('workspace-123', 'OEBPS/styles/main.css');
    });

    it('should handle different EPUB structures', async () => {
      const epubStructures = [
        { basePath: 'OEBPS', href: 'images/cover.jpg', expected: 'OEBPS/images/cover.jpg' },
        { basePath: 'content', href: 'images/cover.jpg', expected: 'content/images/cover.jpg' },
        { basePath: '', href: 'images/cover.jpg', expected: 'images/cover.jpg' }, // OPF in root
        { basePath: 'book/src', href: 'assets/style.css', expected: 'book/src/assets/style.css' }
      ];
  ...
  describe('Storage Backend Integration', () => {
    it('should work with OPFS backend', async () => {
      ...
      const mockFile = new File(['test content'], 'test.jpg', { type: 'wrong/type' });
      mockFileStorage.getFile.mockResolvedValue(mockFile);
      
      const blobURL = await manager.createBlobURL('images/test.jpg');
      
      expect(mockFileStorage.getFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/images/test.jpg');
    ...
    it('should work with IndexedDB backend', async () => {
      ...
      expect(mockFileStorage.readFile).toHaveBeenCalledWith('test-workspace', 'OEBPS/styles/main.css');
      expect(global.URL.createObjectURL).toHaveBeenCalledWith(
        expect.objectContaining({
          type: 'text/css'
        })
    ...
    it('should handle backend detection consistently', () => {
      // OPFS manager
      mockFileStorage.supportsDirectBlobURLs.mockReturnValue(true);
      const opfsManager = new BlobURLManager({
        maxBlobURLs: 100,
        fileStorage: mockFileStorage as any,
        basePath: 'OEBPS',
        onCapacityReached: vi.fn()
      ...
      // IndexedDB manager  
      mockFileStorage.supportsDirectBlobURLs.mockReturnValue(false);
      const indexeddbManager = new BlobURLManager({
        maxBlobURLs: 100,
        fileStorage: mockFileStorage as any,
        basePath: 'OEBPS',
        onCapacityReached: vi.fn()
  ...
  describe('Error Recovery Scenarios', () => {
    ...
    it('should handle capacity exhaustion gracefully', async () => {
      ...
      await expect(manager.createBlobURL('overflow.jpg'))
