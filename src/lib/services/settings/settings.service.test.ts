...
describe('SettingsService Contract Tests', () => {
  ...
  beforeEach(() => {
    ...
    service = new SettingsService(mockFileStorage, mockExtensionManager, mockThemeStore, mockI18nStore);
  ...
  describe('Contract: Infrastructure Integration', () => {
    test('uses FileStorageAPI for workspace and EPUB settings', async () => {
      ...
      expect(mockFileStorage.readTextFile).toHaveBeenCalledWith('test-workspace', '.workspace-metadata.json');
      expect(mockFileStorage.readTextFile).toHaveBeenCalledWith('test-workspace', 'SOURCE/settings.json');
    });
