...
describe.skip('Settings Manager Integration', () => {
  ...
  describe('Edge Cases with Persistence', () => {
    ...
    it('should handle corrupted workspace metadata', async () => {
      // Save valid settings first
      await settingsManager.saveWorkspaceSettings(
        tempWorkspaceId,
        SETTINGS_FIXTURES.workspace.advanced()
      );
    ...
    it('should handle concurrent settings operations', async () => {
      ...
      const isSettings1 =
        JSON.stringify(finalSettings) === JSON.stringify(expect.objectContaining(settings1));
      const isSettings2 =
        JSON.stringify(finalSettings) === JSON.stringify(expect.objectContaining(settings2));
  ...
  describe('Performance and Scale', () => {
    ...
    it('should handle large settings objects', async () => {
      ...
      const largeEPUBSettings = {
        ...
        dom_transforms: Array.from(
          { length: 50 },
          (_, i) => `SOURCE/extensions/transform-${i}/transform.js`
        ),
      };
