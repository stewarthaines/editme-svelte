...
3. **Insertion uses the existing generic `insert` message.** The plugin formats the clip directive itself and sends a plain string; the host's only job is the cursor splice. The plugin carries its own format template as plugin-private config (a single string template, default mirroring the core default directive: `:clip[<label>]{src=<src> begin=<begin> end=<end>}`). This deliberately duplicates the *concept* of the core `audio_clip_template` setting rather than plumbing it through the contract — there are few formats to support, a single template goes a long way, and the panel contract stays at "insert at cursor".
...
- Directive format: a plugin-private single-string template (default `:clip[<label>]{src=<src> begin=<begin> end=<end>}`, matching the core default), editable in a small plugin settings field.
- Persistence: `SOURCE/plugins/audio-clip-editor/clips.json` written via the workspace handle, keyed by audio href, with the format template stored alongside. Schema is plugin-private; the host never reads it.
...
- **Directive format is duplicated, not shared.** The plugin's template and the core `audio_clip_template` setting can drift; a project whose transform expects a customized directive needs the template set in both places. Accepted deliberately to keep the panel contract at "insert at cursor" — revisit only if real projects hit the drift.
