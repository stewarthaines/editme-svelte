...
**Enforcement principle** (settled after weak-model testing): reads are the learning channel and are never gated; writes are the consequential boundary and are gated on the knowledge that makes them safe — the authoring guide for any write, plus (for chapter sources) `seed_get_project_setup` and a read of every configured transform script this session. Refusals are in-band and enumerate exactly what to read, so an agent that skipped the contract is corrected by its first write attempt rather than by a human. Read-gating options (refusing source reads until transforms are read) were considered and rejected: an agent that cannot read freely cannot recover from its own ignorance.

Knowledge tools (**built**, alongside phase 2): `seed_get_authoring_guide` — the bridge process serves `docs/AGENT_AUTHORING.md` (the authoring contract: EPUB CSS fallbacks, progressive-enhancement scripting, the condensed transformText/transformDOM + `ctx` API, Paged.js print quirks, accessibility/epubcheck expectations, write boundaries) before a tab even connects; `seed_get_project_setup` — settings.json with the configured transform list and the read-the-transforms-first hint, because chapter markup is per-project pipeline output. The guide doc is the canonical authoring contract the retired workflow survey called for; future distribution surfaces (context pack, llms.txt, AGENTS.md-in-SOURCE) should serve the same file.

Phase 2 (writes — **built**): **modify-in-place only — debugging existing resources, not content creation.** Implementation notes beyond the design: validation (existence, hash, dirty) runs before the consent prompt _and again after it_ — consent can sit open 90s, and it approves the write that was validated, not whatever the world becomes; the workspace id is pinned at request time so a project switch during the prompt refuses the write; disconnect or teardown cancels open prompts (an orphaned Allow button must not stay actionable). `seed_write_file` may overwrite an **existing, non-generated** file; the boundary is pipeline-shaped, not directory-shaped: the agent writes pipeline _inputs_ and standalone assets (`SOURCE/text/`, `SOURCE/scripts/`, `SOURCE/preview/`, `OEBPS/Styles/`, `OEBPS/Scripts/`, `OEBPS/Images/` and other media), never pipeline _outputs_ (generated `OEBPS/Text/*.xhtml`, the nav) — writing an output invites the delayed-silent-revert trap: the write lands and previews fine, then the next transform run regenerates the file and the work evaporates. Creating files is excluded entirely: a new manifest item is an OPF mutation (entry + derived properties), not a file write — deferred as a future `seed_add_manifest_item` tool wrapping `workspaceService.addManifestItem` (which already handles OPF entry, propagation, and properties). Each write carries the content hash from the agent's prior read, rejected on mismatch ("changed since read"). Writes route through app services so state propagation and track-changes copy-on-write hold by construction; open-editor dirty state refuses the write. OPF/settings stay excluded until service-level mutations exist. Binary overwrites (images, media) get a size cap and a feed line of path + size — the trust surface can't show a reviewable diff for them, stated honestly. Consent steps up: first write per session prompts in the overlay feed (allow once / allow this session).
