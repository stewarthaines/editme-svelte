...
function draw_partempo(){var s,s2,some_part,some_tempo,h,w,y,st,sy=cur_sy
for(st=0;st<=nstaff;st++){if(sy.st_print[st])
break}
if(st>nstaff)
return
...
s2.ymx=s2.ymn+h
anno_start(s2)}
xy_str(s2.x,ymin+2+gene.curfont.pad+gene.curfont.size*.22,s2.text)
...
wlw+=12
break
case'.':case'|':s.x_meter[i]=s.x_meter[i-1]+4.5
break
default:w=0
...
if(parse.state>=2){curvoice=voice_tb[0]
curvoice.eoln=1
sym_link(s)
return}
set_ref(s)
...
if(curvoice&&curvoice.clone){i=parse.eol
parse.eol=parse.bol
do_cloning()
parse.eol=i}
...
break}}
curvoice.time=maxtime;curvoice.eoln=eoln
...
if(p_voice.id==id)
return p_voice}
p_voice={v:v,id:id,time:0,new:true,pos:{},scale:1,ulen:glovar.ulen,dur_fact:1,meter:clone(glovar.meter),wmeasure:glovar.meter.wmeasure,staffnonote:1,clef:{type:C.CLEF,clef_auto:true,clef_type:"a",time:0},acc:[],sls:[],hy_st:0}
...
a_de=[]
cross={}}
function do_cloning(){var i,clone=curvoice.clone,vs=clone.vs,a=clone.a,bol=clone.bol,eol=parse.eol,parse_sav=parse,file=parse.file
...
a_gch.splice(i)}}}
if(curvoice.tr_sco){for(i=0;i<a_gch.length;i++){gch=a_gch[i]
if(gch.type=='g')
...
abc2svg.version="v1.22.22";abc2svg.vdate="2024-11-11"
