...
SEED.html lets you make ebooks — right now, in your browser — that anyone can read. There's
nothing to install and no account to set up. You write in plain text, and SEED.html turns it
into an EPUB, the standard ebook format. You can open that file in the reading apps and devices
you already use, like Apple Books, Google Play Books, Kobo, and Thorium Reader.
...
Calibre or Sigil, you might expect to open any EPUB and edit it. SEED.html works differently — it
can only edit books that were _made_ in SEED.html, because those carry their own source inside
them. You can't open one made elsewhere and edit it here.

The rest of this manual follows the steps of making a book. You'll create a project and write
its chapters, describe the book with its title, author, cover, and other details, check it for
problems and accessibility, and finally produce an EPUB (or PDF) you can share.

> **Try it.** If you're reading this manual inside SEED.html right now, open the
> _Projects_{.ui .icon-house} panel in the sidebar and you'll see this book in the list.
> That's where every book starts.
{.try-it}
...
A SEED EPUB is different. The book you make is an ordinary EPUB any reading app can open, but it
also carries its own source inside, in a small file called `SEED.zip`. That source is what lets
you open the book in SEED.html and edit it again — the book carries everything it needs to be
worked on.
...
This is why SEED.html can only edit books it made: the source travels inside the book. An EPUB
from anywhere else has no source to open.
