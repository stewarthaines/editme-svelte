...
- Read: open a packaged EPUB in the vendored READ.html reader tab (core Publish list + plugin local files + remote files via their public URL, which needs CORS on the remote; HTTP-only); scripted books prompt once per book before their features run
